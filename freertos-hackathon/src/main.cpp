...
/* start runtime statictics collection */
extern "C"
{
...
// Global queues
/**
 * @brief Queue with menu commands from interrupts
 *
 */
QueueHandle_t menu_command_queue;

/**
 * @brief Queue with data to print to menu
 *
 */
QueueHandle_t strings_to_print_queue;

/**
 * @brief Queue with saved EEPROM setpoint
 *
 */
QueueHandle_t sendReadSetpointQueue;

/**
 * @brief Queue with new setpoint to be saved to EEPROM
 *
 */
QueueHandle_t sendNewSetpointToEepromQueue;

/**
 * @brief Declaration of struct variable to hold the menu commands and ticks
 *
 */
